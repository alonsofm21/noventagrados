...
public class NoventaGrados {
	...
	public static void main(String[] args) {
	    // Paso 1: Inicializar la partida y mostrar mensaje de bienvenida
	    inicializarPartida();
	    mostrarMensajeBienvenida();

	    // Paso 2: Bucle principal de la partida
	    while (true) {
	        ...
	        // Paso 3: Validar el formato de la jugada
	        if (!validarFormato(textoJugada)) {
	            mostrarErrorEnFormatoDeEntrada();
	            continue;
	        }

	        // Extraer la jugada en formato de coordenadas
	        ...
	        // Paso 4: Realizar el empujón y cambiar el turno
	        realizarEmpujón(jugada);
	        cambiarTurnoPartida();

	        // Paso 5: Comprobar si la partida ha finalizado
	        if (comprobarFinalizacionPartida()) {
	            // Mostrar el tablero final
	            mostrarTablero();
	            // Mostrar el ganador o indicar empate
	            mostrarGanador();
	            break;
	        }
	    }

	    // Paso 6: Finalizar la partida y cerrar recursos
	    finalizarPartida();
	}
